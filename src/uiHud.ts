...
// Uses a tiny 1-bit bitmap font (fillRect only). No canvas text -> no flicker.

export type UiHudCounts = {
  goslings: number;
  keysCur: number;   // collected (or current)
  keysTotal: number; // total in level
};
...
export function createUiHudSystem(opts: Opts = {}): UiHudSystem {
  const s: State = { goslings: 0, keysCur: 0, keysTotal: 0, t: 0 };
  ...
  function setCounts(next: Partial<UiHudCounts>) {
    ...
    if (next.keysCur != null) s.keysCur = next.keysCur | 0;
    if (next.keysTotal != null) s.keysTotal = next.keysTotal | 0;
  }
  ...
  function draw(ctx: CanvasRenderingContext2D, vw: number, _vh: number, invert: boolean) {
    ...
    // keys: "cur/total"
    const kc = formatCount(s.keysCur);
    const kt = formatCount(s.keysTotal);
    const labelK = `${kc}/${kt}`;

    const glyphGap = (1 * textPx) | 0;
